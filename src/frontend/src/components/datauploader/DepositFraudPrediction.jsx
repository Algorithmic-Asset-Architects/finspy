...
const DepositFraudPrediction = () => {
  ...
  const handlePredict = async () => {
    ...
    try {
      ...
      const response = await axios.post("/api/predict/deposit_fraud/", parsedJson);
  ...
  return (
    <motion.div className="bg-gray-800 p-6 rounded-xl shadow-lg border border-gray-700">
      <h2 className="text-xl font-semibold text-gray-100 mb-4">Deposit Fraud Prediction</h2>
      <textarea
        className="w-full p-3 bg-gray-700 text-white rounded"
        rows="6"
        placeholder="Paste Deposit Fraud JSON here..."
        value={transactionJson}
        onChange={(e) => setTransactionJson(e.target.value)}
      ></textarea>
      <button onClick={handlePredict} className="w-full mt-4 bg-yellow-500 text-white py-2 rounded">
        Predict
      </button>
...
export default DepositFraudPrediction;
